...
def main():
    ...
    # 모델 초기화
    tokenizer, model, image_processor, _ = initialize_model(
        pretrained, model_name, device
    )
    ...
    for video_file in tqdm(video_files):
        ...
        for idx, (start, end) in enumerate(scene_timestamps[video_name]):
            clip_name = (
                f"{os.path.splitext(video_file)[0]}_clip_{start:.3f}-{end:.3f}.mp4"
            )
            clip_id = clip_name[:-4]
            clip_path = os.path.join(clip_output_directory, clip_name)

            prompt = "Please explain the video in detail by referring to the lines, focusing on the action and motion of the video"
            ...
            for script in scripts:
                script_texts[
                    f"{video_name}_clip_{script['start']:.3f}-{script['end']:.3f}"
                ] = (
                    script_texts.get(
                        f"{video_name}_clip_{script['start']:.3f}-{script['end']:.3f}",
                        "",
            ...
            video, frame_time, video_time = load_video(
                clip_path, max_frames_num, fps=fps, force_sample=True
            )
            video = (
            ...
            # 질문 생성 Describe및 출력
            prompt_question = create_question(
                prompt, video_time, len(video[0]), frame_time
            )
            input_ids = (
